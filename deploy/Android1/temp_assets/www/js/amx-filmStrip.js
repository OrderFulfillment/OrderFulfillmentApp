...
/* ----------------------------------------------------------- */(function ()
{
  ...
  filmStrip.prototype.setCurrentPageByIndex = function (amxNode, index)
  {
    ...
    amxNode.setAttributeResolvedValue("__currentPage", di);
    // fire event to notify backend about page change
    if (adf.mf.api.amx.acceptEvent())
    {
      var currentNumberOfItemsPerPage = 0;
      if (pages.childNodes[di] && pages.childNodes[di].firstElementChild)
      {
         var item = pages.childNodes[di].firstElementChild;
         while (item)
         {
      ...
      var pageChangeEvent = new FilmStripPageChangeEvent(di, pageCount, amxNode.getAttribute("__itemsPerPage"), currentNumberOfItemsPerPage);
